...
public struct StatusCodeHandler {
    ...
    /// already a failure
    public let eval: (HTTPURLResponse?) -> Result<HTTPURLResponse, Error>
    ...
    public init(_ eval: @escaping (HTTPURLResponse?) -> Result<HTTPURLResponse, Error>) {
        self.eval = eval
    }
}
...
extension StatusCodeHandler {
    ...
    public static var `default`: StatusCodeHandler {
        return StatusCodeHandler { response in
            guard let httpResponse = response,
                (200...299) ~= httpResponse.statusCode else {
                    return .failure(StatusCodeError(statusCode: response?.statusCode ?? 0, httpResponse: response))
            }
